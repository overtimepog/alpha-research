"""
Command-line interface for EvolveAgent
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="EvolveAgent - Evolutionary coding agent")
    ...
    parser.add_argument(
        "--checkpoint",
        help="Path to checkpoint directory to resume from (e.g., evolve_agent_output/checkpoints/checkpoint_50)",
        default=None,
    )
...
async def main_async() -> int:
    ...
    try:
        ...
        if args.checkpoint:
            ...
            evolve_agent.database.load(args.checkpoint)
            print(
                f"Checkpoint loaded successfully (iteration {evolve_agent.database.last_iteration})"
            )
        ...
        # Run evolution
        best_program = await evolve_agent.run(
            iterations=args.iterations,
            target_score=args.target_score,
        )

        # Get the checkpoint path
        checkpoint_dir = os.path.join(evolve_agent.output_dir, "checkpoints")
